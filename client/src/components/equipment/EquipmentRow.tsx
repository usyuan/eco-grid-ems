...
export function EquipmentRow({ node, onClick }: { node: EquipmentNode; onClick: () => void }) {
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className="flex w-full items-center gap-4 border-b px-4 py-3 text-left transition-colors hover:bg-muted/50"
    >
      ...
      <div className="w-40 shrink-0">
        ...
        <p className="text-xs text-muted-foreground">{TYPE_LABEL[node.type]} · {node.location}</p>
      </div>

      <div className="flex min-w-0 flex-1 items-center gap-2">
        <div className="h-1.5 w-full max-w-40 overflow-hidden rounded-full bg-muted">
          <div
            className={cn("h-full rounded-full", node.status === "critical" ? "bg-destructive" : "bg-primary")}
            style={{ width: `${loadPct}%` }}
          />
        </div>
      ...
      {node.soc !== undefined && (
        <span className="w-16 shrink-0 text-right text-xs tabular-nums text-muted-foreground">SOC {node.soc}%</span>
      )}
