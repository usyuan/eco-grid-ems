  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center gap-3">
        <div className="flex size-12 items-center justify-center rounded-lg bg-surface-hover text-primary">
          <Icon className="size-6" />
        </div>
        <div>
          <p className="text-lg font-semibold">{node.name}</p>
          <p className="text-sm text-muted">{TYPE_LABEL[node.type]} · {node.location}</p>
        </div>
      ...
      <div>
        ...
        <div className="h-2 w-full overflow-hidden rounded-full bg-surface-hover">
          <div
            className={node.status === "critical" ? "h-full rounded-full bg-critical" : "h-full rounded-full bg-primary"}
            style={{ width: `${loadPct}%` }}
          />
        </div>
      </div>
